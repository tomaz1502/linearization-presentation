...
% * As a motivation, on which problems (size, complexity, relevance, ...) does incremental linearisation succeed to give an answer? Can the practical use of the presented work be estimated? (Check Cimatti)
...
\begin{frame}
  ...
  \begin{itemize}
    ...
    \vitem Applications in formal verification of dynamic systems, digital signal processing and motion planning for robots
          ...
          \end{itemize}
    \pause
          \vitem QF\_NRA requires exponential time to solve and QF\_NTA has been shown to be to be undecidable
  \end{itemize}
\end{frame}
...
\begin{frame}
  ...
  \begin{itemize}
    \item \textit{Incremental linearization} is a method introduced by Cimatti et al.\ (2018) for deciding the satisfiability of formulas in QF\_NTA
          \begin{itemize}
...
\begin{frame}
  \frametitle{Incremental Linearization}
  \begin{itemize}
    \item If $\psi'$ is satisfiable, not necessarily $\psi$ is. In this case, the method incrementally introduces
...
\begin{frame}
  ...
  \end{figure}
    ...
    \item Spurious solutions involving transcendental functions are blocked using their \textit{Taylor polynomial}
  \end{itemize}
\end{frame}
...
\begin{frame}
  ...
  \begin{itemize}
    ...
    \pause
    \vitem The solver also defines a \textit{proof calculus} composed of 21 inference rules capturing its reasoning
    \pause
...
\begin{frame}
  ...
  \begin{itemize}
    ...
    \pause
    \vitem Increase confidence in the proof calculus and, once cvc5 emits proofs using the calculus, in its implementation
  \end{itemize}
\end{frame}
...
\begin{center}
  $\infer[]{\forall x \forall y, xy \le bx + ay - ab \leftrightarrow ((x \le a \land y \ge b) \lor (x \ge a \land y \le b))}{- \mid a, b}$
...
\begin{center}
  ...
  $\infer[]{\forall x \forall y, xy \ge bx + ay - ab \leftrightarrow ((x \le a \land y \le b) \lor (x \ge a \land y \ge b))}{- \mid a, b}$
\end{center}
...
\begin{frame}[fragile]
  ...
  \begin{lstlisting}
  theorem arithMulTangentLower (a b x y : ℝ) :
  ...
  theorem arithMulTangentUpper (a b x y : ℝ) :
      x * y ≥ b * x + a * y - a * b ↔ ((x ≤ a ∧ y ≤ b) ∨ (x ≥ a ∧ y ≥ b))
  \end{lstlisting}
...
\begin{frame}[fragile]
  ...
  \begin{lstlisting}
    lemma powNegOdd      : ∀ {k : ℕ} {r : ℝ}, r < 0 → Odd k  → r ^ k < 0
    lemma powNegEven     : ∀ {k : ℕ} {r : ℝ}, r < 0 → Even k → r ^ k > 0
    lemma nonZeroEvenPow : ∀ {k : ℕ} {r : ℝ}, r ≠ 0 → Even k → r ^ k > 0
    lemma powPos         : ∀ {k : ℕ} {r : ℝ}, r > 0           → r ^ k > 0
  \end{lstlisting}

  \begin{lstlisting}
    lemma combineSigns₁ : ∀ {a b : ℝ}, a > 0 → b > 0 → b * a > 0
    lemma combineSigns₂ : ∀ {a b : ℝ}, a > 0 → b < 0 → b * a < 0
    lemma combineSigns₃ : ∀ {a b : ℝ}, a < 0 → b > 0 → b * a < 0
    lemma combineSigns₄ : ∀ {a b : ℝ}, a < 0 → b < 0 → b * a > 0
  \end{lstlisting}
  ...
  \begin{itemize}
    \item Proof reconstruction using \textit{lean-smt}
          \begin{itemize}
                  ...
                  \item lean-smt translates this to a Lean proof and checks it (the Lean theorem is inferred)
                  \item Or one can start with a Lean goal and use lean-smt to translate it to an SMT-LIB query
          \end{itemize}
    \pause
