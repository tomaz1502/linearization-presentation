...
\begin{frame}
  ...
  \begin{itemize}
    ...
    \vitem Applications in formal verification of dynamic systems, digital signal processing and motion planning for robots.
          ...
          \end{itemize}
     \vitem Currently, no solver with complexity better than doubly-exponential for NRA, and NTA has been shown
          to be undecidable.
  \end{itemize}
\end{frame}
...
\begin{frame}
  ...
  \begin{itemize}
    \item \textit{Incremental linearization} is a method introduced by Cimatti et al.\ for deciding the satisfiability of formulas in NTA.
          \begin{itemize}
                  \item Not a decision procedure
          \end{itemize}
    \vitem Given a formula $\psi$ in NTA, a new formula $\psi'$ is obtained by abstracting each transcendental function and variable multiplication as an uninterpreted function.
    \vitem $\psi'$ is thus a formula in UFLRA. If $\psi'$ is unsatisfiable, then so is $\psi$.
  \end{itemize}
\end{frame}
...
\begin{tcolorbox}[colback=green!5,colframe=green!40!black,title=Example]
  ...
  $\psi'$ is the same, but instead of $x^{2}$ and $y^{2}$ we have $f_{*}(x, x)$ e $f_{*}(y, y)$.
...
\begin{frame}
  ...
  \begin{itemize}
    \item cvc5 implements a slightly modified version of this algorithm.
    \vitem The solver also defines a \textit{proof calculus} composed of 21 lemmas capturing its reasoning.
    \vitem These lemmas are, for the most part, the rules that eliminate spurious solutions.
  \end{itemize}
\end{frame}
...
\begin{frame}
  ...
  \begin{itemize}
    \item $\sin$ and $\exp$ are the only ones directly handled.
  ...
  \begin{itemize}
    \item In this work we mechanize proofs of all the lemmas in the proof calculus using the Lean proof assistant.
          \begin{itemize}
                  \item \textit{lean-smt}
          \end{itemize}
    \vitem In this process, we identified missing hypothesis in its documentation, as well as some other inaccuracies.
    \vitem This is the first step towards reconstructing proofs of this algorithm in Lean.
...
\begin{center}
  $\infer[]{m \bowtie 0}{f_{1}, \cdots, f_{k} \mid m}$
\end{center}
...
\begin{frame}[fragile]
  \frametitle{Lemmas for transcendental functions - Bounding $\exp$ e $\sin$}
\begin{center}
...
Where $d$ is congruent to $3$ modulo $4^*$, $\sin$ is concave in the interval $[l, u]^*$ and \texttt{secant}
is the secant line to the $d$-th Taylor polynomial at the points $(l, p(l))$ and $(r, p(r))$.
